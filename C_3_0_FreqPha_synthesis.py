...
from A_2_0_Freq_OpGeData import data_generator, BSdataset_DB_recovery
...
class U2_Net(nn.Module):
    def __init__(self,img_ch=2,output_ch=2):
        ...
        print('It is a U2_net with double ports!')
    ...
    def forward(self,x):
        # encoding path
        # x = x.reshape( x.shape[0],2, 65, 126 )
        x1 = self.Conv1(x)
...
def train(epoch, train_loader, model_student, criterion, optimizer, scheduler):
    ...
    for i, (images, target, mm,nn,kk) in enumerate(train_loader):
        ...
        logits_student, target = Modify(logits_student, target)
...
def validate(epoch, val_loader, model, criterion):
    ...
    with torch.no_grad():
        ...
        for i, (images, target, mm,nn,kk) in enumerate(val_loader):
            ...
            logits, target = Modify(logits, target)
            # measure accuracy and record loss
...
if __name__ == '__main__':
    ...
    trains = BSdataset_DB_recovery(a.trains,preprocess = False)
    train_loader = torch.utils.data.DataLoader(
        trains,
        batch_size=batches, shuffle=True,
        num_workers=workers, pin_memory=True)
    tests = BSdataset_DB_recovery(a.tests,preprocess = False)
    ...
    rnn = nn.DataParallel(U2_Net(),device_ids=[0,1,2,3])
    rnn.to(device)
    print(rnn)
    check = U2_Net()
