...
from A_2_0_Freq_OpGeData import data_generator, BSdataset_BR_recovery
...
class U_Net_Branches(nn.Module):
    def __init__(self,img_ch=2,output_ch=1):
        super(U_Net_Branches,self).__init__()
        ...
        self.Up_conv5 = conv_block(ch_in=1024, ch_out=512)
        #sec. branch
        self.b2_Up5 = up_conv(ch_in=1024,ch_out=512, Size=(8,15))
        self.b2_Up_conv5 = conv_block(ch_in=1024, ch_out=512)

        self.Up4 = up_conv(ch_in=512,ch_out=256, Size=(16,31))
        self.Up_conv4 = conv_block(ch_in=512, ch_out=256)
        #sec. branch
        self.b2_Up4 = up_conv(ch_in=512,ch_out=256, Size=(16,31))
        self.b2_Up_conv4 = conv_block(ch_in=512, ch_out=256)
        
        self.Up3 = up_conv(ch_in=256,ch_out=128, Size=(32,63))
        self.Up_conv3 = conv_block(ch_in=256, ch_out=128)
        #sec. branch
        self.b2_Up3 = up_conv(ch_in=256,ch_out=128, Size=(32,63))
        self.b2_Up_conv3 = conv_block(ch_in=256, ch_out=128) 
        
        self.Up2 = up_conv(ch_in=128,ch_out=64, Size=(65,126))
        self.Up_conv2 = conv_block(ch_in=128, ch_out=64)
        #sec. branch
        self.b2_Up2 = up_conv(ch_in=128,ch_out=64, Size=(65,126))
        self.b2_Up_conv2 = conv_block(ch_in=128, ch_out=64)

        self.Conv_1x1 = nn.Conv2d(64,output_ch,kernel_size=1,stride=1,padding=0)
        #sec. branch
        self.b2_Conv_1x1 = nn.Conv2d(64,output_ch,kernel_size=1,stride=1,padding=0)

    def forward(self,x):
        # encoding path
        # x = x.reshape( x.shape[0],1, 65, 126 )
        # pdb.set_trace()
        # assert( x.shape == torch.tensor((x.shape[0],2, 65, 126)) )
        x1 = self.Conv1(x)
        ...
        # sec. branch: decoding + concat path
        b2_d5 = self.b2_Up5(x5)
        b2_d5 = torch.cat((x4,b2_d5),dim=1)
        b2_d5 = self.b2_Up_conv5(b2_d5)
        
        b2_d4 = self.b2_Up4(b2_d5)
        b2_d4 = torch.cat((x3,b2_d4),dim=1)
        b2_d4 = self.b2_Up_conv4(b2_d4)

        b2_d3 = self.b2_Up3(b2_d4)
        b2_d3 = torch.cat((x2,b2_d3),dim=1)
        b2_d3 = self.b2_Up_conv3(b2_d3)

        b2_d2 = self.b2_Up2(b2_d3)
        b2_d2 = torch.cat((x1,b2_d2),dim=1)
        b2_d2 = self.b2_Up_conv2(b2_d2)

        b2_d1 = self.b2_Conv_1x1(b2_d2)
        b2_d1 = torch.squeeze(b2_d1)
...
def train(epoch, train_loader, model_student, criterion, optimizer, scheduler):
    ...
    for i, (images, target0, target1, mm,nn,kk) in enumerate(train_loader):
        refill = (mm,nn,kk)
        ...
        # compute outputy
        logits_student0, logits_student1 = model_student(images)
        # pdb.set_trace()
        loss = criterion(logits_student0, logits_student1, target0.float(), target1.float())
        logits_student, target = Modify(logits_student0, logits_student1, target0, target1)
...
def validate(epoch, val_loader, model, criterion):
    ...
    with torch.no_grad():
        ...
        for i, (images, target0,target1, mm,nn,kk) in enumerate(val_loader):
            ...
            logits, target = Modify(logits0, logits1, target0, target1)
            # measure accuracy and record loss
...
class BiLoss(nn.Module):
    ...
    def forward(self, prob0, prob1, truth0, truth1):
        ...
        los2 = self.critical(prob1, truth1)*1e5*5.0
        loss = ( los1 + los2 ) / 2.0
        if torch.isnan(loss):
            pdb.set_trace()
        return loss    
...
if __name__ == '__main__':
    ...
    rnn = nn.DataParallel(U_Net_Branches(),device_ids=[0,1,2,3])
    rnn.to(device)
    print(rnn)
    check = U_Net_Branches()
    summary(check, (2, 65,126),device='cpu')
    input = torch.randn(1, 2, 65, 126)
    flops, params = profile(check, inputs=(input, ))
    ...
    for epoch in range(EPOCH):
        ...
        if not ((epoch+1) % 50):
            ssss = 'pause_u_brch_changeloss_'+str(epoch-1)+'.tar'
