...
class data_generator():
    def __init__(self,preprocess=None):
        ...
        self.positive_gen_all(self.bowel_sounds,ROOT_SNRS)
        self.negative_gen_all(ROOT_SNRS)
        self.data_dividing()
    ...
    def positive_gen_all(self,bowels,snrs):
        ...
        for key,value in positive_dis.items():
            ...
            del bowels[0:value]
        self.positive_data = output
        
    def __negative_generate_one__(self,exp,snrs):
        exp = (exp[0].lower(),exp[1])
        aline = [0] * 4000
        quotient = exp[1] // len(snrs)
        remainder = exp[1] % len(snrs)
        ...
        geshu = exp[1] // 5
        assert(exp[1] % 5 == 0)
        random.shuffle(self.noises[exp[0]])
        a_sort = self.noises[exp[0]]
        pick = random.choice(range(len(a_sort)))
        subset = []
        for i in range(geshu):
        ...
        for i in range(len(amount)):
            ...
            for seg in segs:
                ...
                norm_noise[snrs[i]].append( (nenorm,aline,(position,nenorm,(exp[0],snrs[i]),info)) )
        print('Finish the building of NBS segments within the {} noises.'.format(exp[0]))
        return norm_noise
        
    def negative_gen_all(self,snrs):
        #1. assign bowels evenly
        output = {}
        for key,value in negative_dis.items():
            output[key] = self.__negative_generate_one__((key,value),snrs)
        self.negative_data = output
