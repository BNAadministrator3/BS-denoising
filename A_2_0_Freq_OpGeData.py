...
from feature_transformation import read_wav_data, GetFrequencyFeatures, stft
import random
import numpy as np
import torch
import csv
...
class BSdataset(Dataset):
    ...
    def Spec_forming(self, data, fs=4000):
        ...
        for x, y, z in data:
            ...
            # pdb.set_trace()
        print('ALL the spectrograms are generated.')
        return new_data
...
#success
class BSdataset_recovery(Dataset):
    def __init__(self, mydata, transform=None, preprocess = True):
        self.data = self.Spec_forming( mydata )
        self.transform = transform
        self.preprocess = preprocess
    ...
    def Spec_forming(self, data, fs=4000):
        new_data = []
        for x, y, z in data:
            x_abs, x_ang = stft(x, n_fft=128)
            y_abs, y_ang = stft(y, n_fft=128)
            # print(x_abs.shape)
            # print(x_ang.shape)
            # pdb.set_trace()
            # y = GetFrequencyFeatures(y, fs)
            ...
            # assert( isinstance( m, tuple ) )
            # z = ( p,q,m,r )
            new_data.append( (x_abs,y_abs, z, x_ang,y_ang ) )
            # pdb.set_trace()
    ...
    def __getitem__(self, idx):
        ...
        noise_ori = self.data[idx][2][1]
        snr = self.data[idx][2][2][1] 
        xhalves = self.data[idx][3]
        yhalves = self.data[idx][4]
        if self.preprocess:
            x = x.reshape(1,x.shape[0],x.shape[1])
        else:
            pass
        ...
        if self.transform is not None:
            ...
            x = np.array(x)
        y = np.array(y, dtype='float32')
        return torch.from_numpy(x), torch.from_numpy(y), torch.from_numpy(bs_flag), torch.tensor(noise_ori), torch.tensor(snr), torch.tensor(xhalves), torch.tensor(yhalves)
...
#For output two channels 
class BSdataset_DB_recovery(Dataset):
    def __init__(self, mydata, transform=None, preprocess = True):
        self.data = self.Spec_forming( mydata )
        self.transform = transform
        self.preprocess = preprocess
    ...
    def Spec_forming(self, data, fs=4000):
        ...
        for x, y, z in data:
            ...
            x_abs = x_abs.reshape( 1,x_abs.shape[0],x_abs.shape[1] )
            x_ang = x_ang.reshape( 1,x_ang.shape[0],x_ang.shape[1] )
            x_syn = np.concatenate( ( x_abs,x_ang ) )
            
            y_abs = y_abs.reshape( 1,y_abs.shape[0],y_abs.shape[1] )
            y_ang = y_ang.reshape( 1,y_ang.shape[0],y_ang.shape[1] )
            y_syn = np.concatenate( ( y_abs,y_ang ) )
            new_data.append( ( x_syn,y_syn, z ) )
            # pdb.set_trace()
        print('ALL the DB_spectrograms are generated within their phase info.')
        return new_data
...
#For output two channels 
class BSdataset_BR_recovery(Dataset):
    def __init__(self, mydata, transform=None, preprocess = True):
        self.data = self.Spec_forming( mydata )
        self.transform = transform
        self.preprocess = preprocess
    ...
    def Spec_forming(self, data, fs=4000):
        ...
        for x, y, z in data:
            ...
            # y_ang = y_ang.reshape( 1,y_ang.shape[0],y_ang.shape[1] )
            # y_syn = np.concatenate( ( y_abs,y_ang ) )
            new_data.append( ( x_syn, (y_abs, y_ang), z ) )
            # pdb.set_trace()
        print('ALL the DB_spectrograms are generated within their phase info.')
        return new_data
    ...
    def __getitem__(self, idx):
        ...
        if self.transform is not None:
            x = self.transform(x)
            x = np.array(x)
        y0 = np.array(y0, dtype='float32')
        y1 = np.array(y1, dtype='float32')
        return torch.from_numpy(x), torch.from_numpy(y0), torch.from_numpy(y1), torch.from_numpy(bs_flag), torch.tensor(noise_ori), torch.tensor(snr)
