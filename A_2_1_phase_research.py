...
from feature_transformation import read_wav_data, GetFrequencyFeatures, stft, istft 
import random
import numpy as np
import torch
import csv
...
class BSdataset_BR_recovery(Dataset):
    ...
    def Spec_forming(self, data, fs=4000):
        ...
        for x, y, z in data:
            ...
            new_data.append( ( ( x_syn ,x ), (y_abs, y_ang, y), z ) )
            # pdb.set_trace()
        print('ALL the BR_spectrograms are generated within their phase info.')
        return new_data
...
if __name__ == '__main__':
    ...
    for datum in okay:
        if datum[2][2][1] == ROOT_SNRS[i]:
            ...
            coutour=plt.subplot(414)
            coutour=plt.pcolormesh(phas)
            plt.colorbar(coutour, label = "power (dB)")
