...
from A_2_0_Freq_OpGeData import data_generator, BSdataset
...
def __sdrComputing__( pred, truth ):
    try:
        pred.shape == truth.shape
    except:
        pdb.set_trace()
    de = np.linalg.norm(pred-truth)
    no = np.linalg.norm(truth)
    result = 20 * log( no/de + eps )
    return result
    
def SDR(logits_student, target):
    num = logits_student.shape[0]
    crs = []
    for i in range(num):
        temp = __sdrComputing__( logits_student[i], target[i] )
        crs.append( temp )
    return np.mean(crs)
    ...
    with torch.no_grad():
        ...
        for z in bsflag:
            if np.sum(z) == 0:
                ...
                truth.append( 1 )
        ratio = SDR(logits_student, target)
        return ratio
...
def train(epoch, train_loader, model_student, criterion, optimizer, scheduler):
    ...
    top5 = AverageMeter('SDR@5', ':6.2f')

    progress = ProgressMeter(
        len(train_loader),
        [batch_time, data_time, losses, top5],
    ...
    for i, (images, target, mm,nn,kk) in enumerate(train_loader):
        ...
        ratio = measure(logits_student, target, refill)
...
def validate(epoch, val_loader, model, criterion):
    ...
    with torch.no_grad():
        ...
        for i, (images, target, mm,nn,kk) in enumerate(val_loader):
            ...
            ratio = measure_val(logits, target, refill)
            n = images.size(0)
        ...
        print(' * SDR@5 {top5.avg:.3f}'
              .format(top5=top5))
        

    return losses.avg, top5.avg #, top5.avg  
...
if __name__ == '__main__':
    ...
    print(rnn)
    check = RNN()
    summary(check, (10,200),device='cpu')
    input = torch.randn(1, 10, 200)
